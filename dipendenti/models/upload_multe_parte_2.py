...
# Apri il file Excel
workbook = openpyxl.load_workbook('./trattenute_multe.xlsx')
...
response1 = input("Hai creato gli utenti 'Petit', 'Cardone Gerardo', 'Milkman'? Si/No")
response2 = input("Hai messo nel file excel IDTRATTENUTE in ordine crescente? Si/No")

if response1.lower() != "si" and response2.lower() != "si":
    quit()


for record in data:
    if record['IDTRATTENUTE'] <= 1166:
        continue
    PRINT("\nSTAMPO RECORD\n\n")
    PRINT(record)
    anomalia_id = record['ID_ODOO']
    ...
    if soggetto_trattenuta_id == []:
        SALVA_LOG("multe_trattenute_saltate.txt", f"Il dipendente {soggetto_trattenuta} non esiste nei res.partner\n")
        continue
    ...
    PRINT(f"fleet_vehicle_log_service_id = {anomalia_id}")
    ...
    if datetime_trattenuta != None:
        ...
        COCOZZA_connectDb.sqlCreate('deduction.deduction', {
            'fleet_vehicle_log_service_id': anomalia_id,
